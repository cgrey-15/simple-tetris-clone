...
RudimentarySprite::RudimentarySprite(Object2D& object, Magnum::Int width, Magnum::Int height, Magnum::Float frameDuration, Magnum::GL::Texture2D& texture, Magnum::Timeline& timeline,Magnum::SceneGraph::DrawableGroup2D& group, Magnum::Shaders::Flat2D& shader ) :
...
void RudimentarySprite::getPrevTime() {
	_prevUpdatePoint = _tl.previousFrameTime();
}

void RudimentarySprite::setColor(Magnum::Color3ub color)
{
	_color = color;
}

RudimentarySprite& RudimentarySprite::setVisibility(const bool isVisible) {
	_visible = isVisible;
	return *this;
}

RudimentarySprite& RudimentarySprite::setTexCoords(const std::initializer_list<Magnum::Vector2>& list) {
	_texCoords = Corrade::Containers::Array<Magnum::Vector2>(Corrade::Containers::InPlaceInit, list);
	_size = _texCoords.size();
	return *this;
}

RudimentarySprite& RudimentarySprite::setTexCoords(Corrade::Containers::Array<Magnum::Vector2>&& vertices) {
	_texCoords = Corrade::Containers::Array<Magnum::Vector2>(std::move(vertices));
	_size = _texCoords.size();
	return *this;
}

RudimentarySprite& RudimentarySprite::setTexCoords(Magnum::Vector2 singles) {
	_texCoords = Corrade::Containers::Array<Magnum::Vector2>(Corrade::Containers::InPlaceInit, {singles});
	_size = _texCoords.size();
	return *this;
}

void RudimentarySprite::draw(const Magnum::Matrix3& transform, Magnum::SceneGraph::Camera2D& camera) {
